...
class InternVL3Embedder(nn.Module):
    ...
    def _prepare_and_fuse_embeddings(self, prompt: str, vit_embeds: torch.Tensor, image_mask: torch.Tensor, num_tiles_list: List[int]):
        ...
        img_token_mask = (input_ids == self.img_context_token_id) # shape (1, N)
        img_token_locations = torch.where(img_token_mask)[1] # shape (num_true_values,)
        input_embeds = self.model.language_model.get_input_embeddings()(input_ids).clone() # (1, N, C)
        B, N, C = input_embeds.shape
        input_embeds = input_embeds.reshape(B * N, C)
        input_ids = input_ids.reshape(B * N)

        ## 替换图像token的embedding
        selected = (input_ids == self.img_context_token_id) # shape (N,)
        try:
        ...
        ## 处理 attention mask，屏蔽掉被mask的图像对应的token
        current_token_idx = 0
    ...
    def get_fused_image_text_embedding_from_tensor_images(self, image, image_mask, text_prompt):
        pixel_values, num_tiles_list = self._preprocess_images(image)
        if pixel_values.shape[0] == 0:
            print("Warning: No valid images to process after masking.")

        vit_embeds = self.model.extract_feature(pixel_values) # (total_tiles, seq_len, vit_embed_dim)
        ...
        return fused_hidden, attention_mask
